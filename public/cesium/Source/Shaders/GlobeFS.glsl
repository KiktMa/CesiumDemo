...
float interpolateByDistance(vec4 nearFarScalar,float distance)
{
    ...
    float t=clamp((distance-startDistance)/(endDistance-startDistance),0.,1.);
...
{
    return sourceColor*vec4(sourceColor.aaa,1.)+destinationColor*(1.-sourceColor.a);
}
#endif
...
void main()
{
    ...
    #if defined(APPLY_DAY_NIGHT_ALPHA)&&defined(ENABLE_DAYNIGHT_SHADING)
    float nightBlend=1.-clamp(czm_getLambertDiffuse(czm_lightDirectionEC,normalEC)*5.,0.,1.);
    #else
    float nightBlend=0.;
    #endif
    ...
    vec4 color=computeDayColor(u_initialColor,clamp(v_textureCoordinates,0.,1.),nightBlend);
    ...
    float diffuseIntensity=clamp(czm_getLambertDiffuse(czm_lightDirectionEC,normalize(v_normalEC))*.9+.3,0.,1.);
    vec4 finalColor=vec4(color.rgb*czm_lightColor*diffuseIntensity,color.a);
    #elif defined(ENABLE_DAYNIGHT_SHADING)
    float diffuseIntensity=clamp(czm_getLambertDiffuse(czm_lightDirectionEC,normalEC)*5.+.3,0.,1.);
    ...
    float darken=clamp(dot(normalize(czm_viewerPositionWC),atmosphereLightDirection),u_minimumBrightness,1.);
    fogColor*=darken;
    #endif
    ...
    if(czm_backFacing())
    {
        float distanceFromEllipsoid=max(czm_eyeHeight,0.);
        float distance=max(v_distance-distanceFromEllipsoid,0.);
...
vec3 computeGroundAtmosphereColor(vec3 fogColor,vec4 finalColor,vec3 atmosphereLightDirection,float cameraDist)
{
    ...
    float mpp=czm_metersPerPixel(vec4(0.,0.,-czm_currentFrustum.x,1.),1.);
...
vec4 computeWaterColor(vec3 positionEyeCoordinates,vec2 textureCoordinates,mat3 enuToEye,vec4 imageryColor,float maskValue,float fade)
{
    ...
    float specularIntensity=czm_getSpecular(czm_lightDirectionEC,normalizedPositionToEyeEC,normalEC,10.);
